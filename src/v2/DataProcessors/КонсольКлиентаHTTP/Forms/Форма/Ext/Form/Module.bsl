...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	AWSРегион = Элементы.РегионAWS.СписокВыбора[0];
...
Процедура ДобавитьВИсторию(Знач Ответ)
	...
	ОбъектИстории.Вставить("МассивКодовСостоянияОтветаПовторногоВыполнения", МассивКодовСостоянияОтветаПовторногоВыполнения());
...
&НаКлиенте
Функция ПарольныйВидТекста(Знач Текст, Знач ЭлементФормы, Знач СкрыватьПустуюСтроку = Истина)
	Возврат ?(
		НЕ СкрыватьПустуюСтроку И Текст = "",
		"",
		?(ЭлементФормы.РежимПароля, "***", Текст)
	);
КонецФункции
...
Процедура ДобавитьВКодЗапросаИспользованиеПрокси(Построитель, ЕстьДополнительныеПараметры)
	...
	Хвост = ?(
			...
			?(ПустаяСтрока(Хвост), "", ", "),
			", """ + КавычкиВТексте1С(ПарольныйВидТекста(ПарольПрокси, Элементы.ПарольПрокси)) + """"
		) + Хвост;
	Хвост = ?(
...
Процедура ДобавитьВКодЗапросаСертификаты(Построитель, ЕстьФайлКлиентскогоСертификата, ЕстьФайлСертификатовУЦ, ЕстьДополнительныеПараметры)
	...
	Если ПарольКлиентскогоСертификата <> "" Тогда
		ЧастиСтроки.Добавить(СтрШаблон(
			"""%1""",
			КавычкиВТексте1С(ПарольныйВидТекста(ПарольКлиентскогоСертификата, Элементы.ПарольКлиентскогоСертификата))
		));
	КонецЕсли;
	Построитель.Добавить(СтрСоединить(ЧастиСтроки, ", ") + ")");
	...
	Если ПарольФайлаСертификатовУЦ <> "" Тогда
		ЧастиСтроки.Добавить(СтрШаблон(
			"""%1""",
			КавычкиВТексте1С(ПарольныйВидТекста(ПарольФайлаСертификатовУЦ, Элементы.ПарольФайлаСертификатовУЦ))
		));
	КонецЕсли;
	Построитель.Добавить(СтрСоединить(ЧастиСтроки, ", ") + ")");
КонецПроцедуры
...
Процедура ДобавитьВКодЗапросаАутентификацию(Построитель, ЕстьДополнительныеПараметры)
	...
	Если ТипАутентификации = ТипАутентификацииСписокВыбора[1].Значение Тогда // Basic
		Построитель.Добавить(СтрШаблон(
			...
			КавычкиВТексте1С(ПарольныйВидТекста(ПарольАутентификации, Элементы.ПарольАутентификации))
		));
	...
	ИначеЕсли ТипАутентификации = ТипАутентификацииСписокВыбора[4].Значение Тогда // Bearer
		Построитель.Добавить(СтрШаблон(
			...
			ПарольныйВидТекста(ТокенАутентификации, Элементы.ТокенАутентификации)
		));
	...
	ИначеЕсли ТипАутентификации = ТипАутентификацииСписокВыбора[5].Значение Тогда // AWS4-HMAC-SHA256
		...
		ЧастиСтроки.Добавить(СтрШаблон(
			...
			AWSКлючДоступа,
			КавычкиВТексте1С(ПарольныйВидТекста(AWSСекретныйКлюч, Элементы.СекретныйКлючAWS))
		));
	...
	ИначеЕсли ТипАутентификации = ТипАутентификацииСписокВыбора[6].Значение Тогда // Hawk
		...
		ЧастиСтроки.Добавить(СтрШаблон(
			...
			HawkИдентификатор,
			КавычкиВТексте1С(ПарольныйВидТекста(HawkКлюч, Элементы.КлючHawk))
		));
...
Процедура ДобавитьВКодЗапросаИнициализациюКонфигурации(Построитель, Знач ЕстьПараметрыЗапроса, Знач ЕстьДополнительныеПараметры, Знач ТребуетсяИмяФайлаТелаОтвета, Знач ЕстьТекстТелаЗапроса, Знач ЕстьПоляФормы, Знач ЕстьФайлКлиентскогоСертификата, Знач ЕстьФайлСертификатовУЦ)
	Если ЕстьФайлСертификатовУЦ Тогда
		Построитель.Вставить(1, СтрШаблон("ФайлСертификатовУЦ = ""%1"";", ИмяФайлаСертификатовУЦ));
	КонецЕсли;
	
	Если ЕстьФайлКлиентскогоСертификата Тогда
		Построитель.Вставить(1, СтрШаблон("ФайлКлиентскогоСертификата = ""%1"";", ИмяФайлаКлиентскогоСертификата));
	КонецЕсли;
...
Процедура ЗаполнитьКлиентскийСертификатCurlКоманды(ЧастиКоманды)
	...
	Пароль = ПарольныйВидТекста(ПарольКлиентскогоСертификата, Элементы.ПарольКлиентскогоСертификата, Ложь);
	Если Пароль <> "" Тогда
...
Процедура ЗаполнитьПользователяИПарольCurlКоманды(ЧастиКоманды)
	...
	ЧастиАутентификации.Добавить(ПарольныйВидТекста(ПарольАутентификации, Элементы.ПарольАутентификации));
...
Процедура ЗаполнитьAWS4CurlКоманды(ЧастиКоманды)
	...
	ЧастиАутентификации.Добавить(ПарольныйВидТекста(AWSСекретныйКлюч, Элементы.СекретныйКлючAWS));
	ЧастиКоманды.Добавить(СтрШаблон("'%1'", ЭкранированиеАпострофаUnix(СтрСоединить(ЧастиАутентификации, ":"))));
...
Процедура ЗаполнитьАутентификациюCurlКоманды(ЧастиКоманды)
	...
	ИначеЕсли ТипАутентификации = Элементы.ТипАутентификации.СписокВыбора[4].Значение Тогда // Bearer
		...
		ЧастиКоманды.Добавить(СтрШаблон("'%1'", ЭкранированиеАпострофаUnix(ПарольныйВидТекста(ТокенАутентификации, Элементы.ТокенАутентификации))));
...
Процедура ЗаполнитьПроксиCurlКоманды(ЧастиКоманды)
	...
	Если ЗначениеЗаполнено(ПользовательПрокси) Тогда
		...
		ЧастиПрокси.Добавить(ПарольныйВидТекста(ПарольПрокси, Элементы.ПарольПрокси));
...
&НаКлиенте
Функция МассивКодовСостоянияОтветаПовторногоВыполнения()
	Возврат КодыСостоянияОтветаПовторногоВыполнения.ВыгрузитьЗначения();
КонецФункции
