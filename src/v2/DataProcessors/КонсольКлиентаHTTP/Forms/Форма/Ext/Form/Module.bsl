...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Дополнительно.Сессия = Новый Структура("Печенье", Новый Соответствие);
...
Процедура ТипАвторизацииПриИзменении(Элемент)
	...
	ОтображатьAWS4 = (ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[5].Значение);
...
Функция ДополнительныеПараметрыЗапроса()
	...
	ИначеЕсли ТипАвторизации = ТипАвторизацииСписокВыбора[5].Значение Тогда // AWS4-HMAC-SHA256
...
Процедура ЗаполнитьПользователяИПарольCurlКоманды(ЧастиКоманды)
	Если НЕ ЗначениеЗаполнено(ПользовательАвторизации) Тогда
		Возврат;
	КонецЕсли;
	...
	ЧастиАвторизации.Добавить(ПользовательАвторизации);
	ЧастиАвторизации.Добавить(ПарольАвторизации);
	ЧастиКоманды.Добавить(СтрШаблон("'%1'", ЭкранированиеАпострофаUnix(СтрСоединить(ЧастиАвторизации, ":"))));
КонецПроцедуры
...
Процедура ЗаполнитьAWS4CurlКоманды(ЧастиКоманды)
	...
	ЧастиКоманды.Добавить("-u");
	ЧастиАвторизации.Добавить(AWSКлючДоступа);
	ЧастиАвторизации.Добавить(AWSСекретныйКлюч);
	ЧастиКоманды.Добавить(СтрШаблон("'%1'", ЭкранированиеАпострофаUnix(СтрСоединить(ЧастиАвторизации, ":"))));
	
	ЧастиКоманды.Добавить("--aws-sigv4");
	ЧастиАвторизации.Очистить();
	ЧастиАвторизации.Добавить("aws:amz");
	Если НЕ ПустаяСтрока(AWSРегион) Тогда
		ЧастиАвторизации.Добавить(AWSРегион);
		Если НЕ ПустаяСтрока(AWSСервис) Тогда
			ЧастиАвторизации.Добавить(AWSСервис);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьАутентификациюCurlКоманды(ЧастиКоманды)
	Если ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[1].Значение Тогда // Basic
		ЗаполнитьПользователяИПарольCurlКоманды(ЧастиКоманды);
		ЧастиКоманды.Добавить("--basic");
	ИначеЕсли ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[2].Значение Тогда // NTLM
		ЗаполнитьПользователяИПарольCurlКоманды(ЧастиКоманды);
		ЧастиКоманды.Добавить("--ntlm");
	ИначеЕсли ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[3].Значение Тогда // Digest
		ЗаполнитьПользователяИПарольCurlКоманды(ЧастиКоманды);
		ЧастиКоманды.Добавить("--digest");
	ИначеЕсли ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[4].Значение Тогда // Bearer
		ЧастиКоманды.Добавить("--oauth2-bearer");
		ЧастиКоманды.Добавить(СтрШаблон("'%1'", ЭкранированиеАпострофаUnix(ТокенАвторизации)));
	ИначеЕсли ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[5].Значение Тогда // AWS4-HMAC-SHA256
...
Функция АвторизацияИмпортаPostman(Знач Запрос, Знач ЗначенияШаблонов)
	...
	фРезультат = Новый Структура(
		"Тип, Пользователь, Пароль, Токен, РегионAWS, СервисAWS",
		Элементы.ТипАвторизации.СписокВыбора[0], "", "", "", "", ""
	);
	...
	Для Каждого ЭлементТиповАвторизации Из Элементы.ТипАвторизации.СписокВыбора Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если НайденныйТипАвторизации = Неопределено И ТипАвторизацииИмпорт = "AWSV4" Тогда
		НайденныйТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[5]; // AWS4-HMAC-SHA256
	КонецЕсли;
	...
	Имена = СтрРазделить("Пароль,Пользователь,Токен,Пользователь,Пароль,РегионAWS,СервисAWS", ",");
	ИменаИмпорт = СтрРазделить("password,username,token,accesskey,secretkey,region,service", ",");
...
Процедура ИмпортироватьВыбранныйЭлементИмпортаPostman(Знач ОбъектИмпорта, Знач ПеременныеИмпорта, Отказ, ОписаниеОшибки)
	...
	ТипАвторизации = АвторизацияИмпорт.Тип.Значение;
	Если ТипАвторизации <> Элементы.ТипАвторизации.СписокВыбора[0].Значение Тогда
		СписокТиповАвторизации = Элементы.ТипАвторизации.СписокВыбора;
		Если ТипАвторизации = СписокТиповАвторизации[1].Значение // Basic
			ИЛИ ТипАвторизации = СписокТиповАвторизации[2].Значение // NTLM
			ИЛИ ТипАвторизации = СписокТиповАвторизации[3].Значение // Digest
		Тогда
			ПользовательАвторизации = АвторизацияИмпорт.Пользователь;
			ПарольАвторизации = АвторизацияИмпорт.Пароль;
		ИначеЕсли ТипАвторизации = СписокТиповАвторизации[4].Значение Тогда // Bearer
			ТокенАвторизации = АвторизацияИмпорт.Токен;
		ИначеЕсли ТипАвторизации = СписокТиповАвторизации[5].Значение Тогда // AWS4-HMAC-SHA256
			AWSКлючДоступа = АвторизацияИмпорт.Пользователь;
			AWSСекретныйКлюч = АвторизацияИмпорт.Пароль;
			AWSРегион = АвторизацияИмпорт.РегионAWS;
			AWSСервис = АвторизацияИмпорт.СервисAWS;
		КонецЕсли;
	КонецЕсли;
...
Функция НовыйОбъектДанныхCurl()
	Возврат Новый Структура(
		...
		Новый Массив,
		Ложь,
		Новый Структура("Тип, Пользователь, Пароль, Токен, РегионAWS, СервисAWS", Элементы.ТипАвторизации.СписокВыбора[0], "", "", "", "", ""),
		Ложь,
		Неопределено,
		Ложь,
		Неопределено,
...
Процедура ОбработатьДанныеАвторизацииCurl(ДанныеИмпорта, Знач Опция)
	...
	ДанныеИмпорта.Авторизация.Пароль = СтрСоединить(ЧастиЗначения, ":");
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьДанныеАвторизацииAWSCurl(ДанныеИмпорта, Знач Опция)
	ЗначениеОпции = Опция.Значение;
	Если НЕ ЗначениеЗаполнено(ЗначениеОпции) Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеИмпорта.Авторизация.Тип = Элементы.ТипАвторизации.СписокВыбора[5];
	
	Если ЧастиЗначения.Количество() > 2 Тогда
		ДанныеИмпорта.Авторизация.РегионAWS = ЧастиЗначения[2];
	КонецЕсли;
	Если ЧастиЗначения.Количество() > 3 Тогда
		ДанныеИмпорта.Авторизация.СервисAWS = ЧастиЗначения[3];
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьОпциюCurl(ДанныеИмпорта, Знач ИдентификаторРесурсаИмпорт, Знач Опция, Отказ, ОписаниеОшибки)
	...
	ИначеЕсли ИмяОпции = "--aws-sigv4" Тогда
		ОбработатьДанныеАвторизацииAWSCurl(ДанныеИмпорта, Опция);
...
Асинх Процедура ПрочитатьКомандуCurl(СтрокаКоманды)
	...
	ТипАвторизации = ДанныеИмпорта.Авторизация.Тип.Значение;
	Если ТипАвторизации <> Элементы.ТипАвторизации.СписокВыбора[0].Значение Тогда
		СписокТиповАвторизации = Элементы.ТипАвторизации.СписокВыбора;
		Если ТипАвторизации = СписокТиповАвторизации[1].Значение // Basic
			ИЛИ ТипАвторизации = СписокТиповАвторизации[2].Значение // NTLM
		...
		ИначеЕсли ТипАвторизации = СписокТиповАвторизации[5].Значение Тогда // AWS4-HMAC-SHA256
			AWSКлючДоступа = ДанныеИмпорта.Авторизация.Пользователь;
			AWSСекретныйКлюч = ДанныеИмпорта.Авторизация.Пароль;
			AWSРегион = ДанныеИмпорта.Авторизация.РегионAWS;
			AWSСервис = ДанныеИмпорта.Авторизация.СервисAWS;
		КонецЕсли;
	КонецЕсли;
