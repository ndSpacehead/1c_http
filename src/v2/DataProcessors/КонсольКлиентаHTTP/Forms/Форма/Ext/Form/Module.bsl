...
Процедура ТипАутентификацииПриИзменении(Элемент)
	...
	ОтображатьAWS4 = (ТипАутентификации = Элементы.ТипАутентификации.СписокВыбора[5].Значение);
	ОтображатьHawk = (ТипАутентификации = Элементы.ТипАутентификации.СписокВыбора[6].Значение);
...
Процедура ПроверитьОтветHawk(Знач Ответ)
	...
	ИмяЗаголовка = "";
	Для Каждого КЗ Из Ответ.Заголовки Цикл
		Если СтрНайти(НРег(КЗ.Ключ), "auth") > 0 И НРег(Лев(КЗ.Значение, 4)) = "hawk" Тогда
			ИмяЗаголовка = КЗ.Ключ;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если ИмяЗаголовка = "" Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеОшибки = "";
	Если НЕ КлиентHTTPКлиентСервер.ОтветСЗаголовкомHawkКорректен(Ответ, ИмяЗаголовка, ОписаниеОшибки) Тогда
...
Функция ДополнительныеПараметрыЗапроса()
	...
	ИначеЕсли ТипАутентификации = ТипАутентификацииСписокВыбора[6].Значение Тогда // Hawk
		КлиентHTTPКлиентСервер.УстановитьHawkАвторизацию(фРезультат, HawkИдентификатор, HawkКлюч);
	КонецЕсли;
...
Функция АутентификацияИмпортаPostman(Знач Запрос, Знач ЗначенияШаблонов)
	...
	фРезультат = Новый Структура(
		...
		Элементы.ТипАутентификации.СписокВыбора[0], "", "", "", "", ""
	);
	...
	Для Каждого ЭлементТиповАутентификации Из Элементы.ТипАутентификации.СписокВыбора Цикл
	...
	Если НайденныйТипАутентификации = Неопределено И ТипАутентификацииИмпорт = "AWSV4" Тогда
		НайденныйТипАутентификации = Элементы.ТипАутентификации.СписокВыбора[5]; // AWS4-HMAC-SHA256
	КонецЕсли;
