...
Процедура ОтобразитьТекстОтвета(Знач Данные, Знач Заголовки)
	...
	ИначеЕсли ТипMIME = КлиентHTTPПовтИсп.ТипMIMETextXML() Тогда
		ФорматироватьТекстТелаОтвета = Истина;
...
Функция КомандаCurlЗапросаКонсоли()
	...
	Если ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[1].Значение Тогда // Basic
		Если ЗначениеЗаполнено(ПользовательАвторизации) Тогда
			ЧастиКоманды.Добавить("-u");
			ЧастиАвторизации.Добавить(ПользовательАвторизации);
			ЧастиАвторизации.Добавить(ПарольАвторизации);
			ЧастиКоманды.Добавить(СтрШаблон("'%1'", СтрЗаменить(СтрСоединить(ЧастиАвторизации, ":"), "'", "'\''")));
		КонецЕсли;
	...
	ИначеЕсли ТипАвторизации = Элементы.ТипАвторизации.СписокВыбора[4].Значение Тогда // Bearer
		...
		ЧастиКоманды.Добавить(СтрЗаменить(СтрШаблон("'%1'", ТокенАвторизации), "'", "'\''"));
	КонецЕсли;
	...
	Если ТипТелаЗапроса = Элементы.ТипТелаЗапроса.СписокВыбора[1].Значение Тогда // multipart/form-data
		...
		Для Каждого Стр Из ТелоЗапросаДанныеФормы Цикл
			ТипMIME = "";
			ИмяФайла = "";
			ЗаменаОбратногоСлеша = "\\";
			ЭтоФайл = Стр.Тип = ТипыДанныхФормыФайл;
			ЧастиПоляФормы.Очистить();
			ЧастиПоляФормы.Добавить(СтрЗаменить(Стр.Ключ, "'", "'\''"));
			...
			Если ЭтоФайл Тогда
				...
				ТипMIME = КлиентHTTPСлужебный.ТипMIMEРасширенияФайла(ФайлПоля.Расширение);
				Если ?(ВыполнятьНаСервере, РазделительПутиСервера(), ПолучитьРазделительПутиКлиента()) = "\" Тогда
					ЗаменаОбратногоСлеша = "\";
				КонецЕсли;
			КонецЕсли;
			
			ЧастиПоляФормы.Добавить("""");
			ЧастиПоляФормы.Добавить(СтрЗаменить(СтрЗаменить(СтрЗаменить(Стр.Значение, "\", ЗаменаОбратногоСлеша), "'", "'\''"), """", "\"""));
		...
		ПредопределенныеЗаголовки.Добавить(Новый Структура("Ключ, Значение", "Content-Type", "multipart/form-data; boundary=" + Разделитель));
	ИначеЕсли ТипТелаЗапроса = Элементы.ТипТелаЗапроса.СписокВыбора[2].Значение Тогда // application/x-www-form-urlencoded
		...
		Для Каждого Стр Из ТелоЗапросаДанныеHTMLФормы Цикл
			...
			ЧастиКоманды.Добавить(СтрШаблон("'%1=%2'", КодированныйТекстВURL(Стр.Ключ), СтрЗаменить(Стр.Значение, "'", "'\''")));
		КонецЦикла;
	...
	ИначеЕсли ТипТелаЗапроса = Элементы.ТипТелаЗапроса.СписокВыбора[3].Значение Тогда // Текст
		...
		Иначе
			...
			ЧастиКоманды.Добавить(СтрШаблон("'%1'", СтрЗаменить(ТелоЗапросаТекст, "'", "'\''")));
		КонецЕсли;
	ИначеЕсли ТипТелаЗапроса = Элементы.ТипТелаЗапроса.СписокВыбора[4].Значение Тогда // Файл
		...
		Иначе
			...
			ЧастиКоманды.Добавить(СтрШаблон("@""%1""", СтрЗаменить(ТелоЗапросаФайл, "'", "'\''")));
	...
	Если ТелоОтветаВФайл И ЗначениеЗаполнено(ИмяФайлаТелаОтвета) Тогда
		ЧастиКоманды.Добавить("-o");
		ЧастиКоманды.Добавить(СтрШаблон("""%1""", ИмяФайлаТелаОтвета));
	КонецЕсли;
	
	// Прокси
	Если ИспользоватьПрокси И ЗначениеЗаполнено(СерверПрокси) Тогда
	...
	Если ИспользоватьСессию Тогда
		...
		Если ПоддержкаCookie Тогда
			...
			Если ЗначениеЗаполнено(ЗначениеCookie) Тогда
				...
				ЧастиКоманды.Добавить(СтрШаблон("'%1'", СтрЗаменить(ЗначениеCookie, "'", "'\''")));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	// Заголовки
	Для Каждого ПредопределенныйЗаголовок Из ПредопределенныеЗаголовки Цикл
		ЧастиКоманды.Добавить("-H");
		ЧастиКоманды.Добавить(СтрШаблон("'%1: %2'", СтрЗаменить(ПредопределенныйЗаголовок.Ключ, "'", "'\''"), СтрЗаменить(ПредопределенныйЗаголовок.Значение, "'", "'\''")));
	КонецЦикла;
	Для Каждого Стр Из ЗаголовкиЗапроса Цикл
		Если Стр.Активно Тогда
			ЧастиКоманды.Добавить("-H");
			ЧастиКоманды.Добавить(СтрШаблон("'%1: %2'", СтрЗаменить(Стр.Ключ, "'", "'\''"), СтрЗаменить(Стр.Значение, "'", "'\''")));
		КонецЕсли;
	КонецЦикла;
	
	ЧастиКоманды.Вставить(1, КодированныйURIВURL(ИдентификаторРесурса));
	
	// Тип запроса
	Если Схема = Элементы.Схема.СписокВыбора[0].Значение И НЕ ОтобразитьPOST Тогда // GET
		ЧастиКоманды.Вставить(1, СтрШаблон("-X %1", Схема));
	ИначеЕсли Схема = Элементы.Схема.СписокВыбора[1].Значение И ОтобразитьPOST Тогда // POST
		ЧастиКоманды.Вставить(1, СтрШаблон("-X %1", Схема));
	ИначеЕсли Схема = Элементы.Схема.СписокВыбора[2].Значение Тогда // PUT
		ЧастиКоманды.Вставить(1, СтрШаблон("-X %1", Схема));
	ИначеЕсли Схема = Элементы.Схема.СписокВыбора[3].Значение Тогда // DELETE
	...
	Если ЧастиСложнойОпции.Количество() > 0 Тогда
		...
		ЧастиКоманды.Вставить(1, СтрСоединить(ЧастиСложнойОпции, ""));
	КонецЕсли;
