...
Процедура ДанныеФормыТипПриИзменении(Элемент)
	...
	Элемент.Родитель.ТекущиеДанные.ТипКонтента = "";
	Элемент.Родитель.ТекущиеДанные.ИмяФайла = "";
КонецПроцедуры
...
&НаКлиенте
Асинх Процедура ДанныеФормыТипКонтентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекущиеДанные = Элемент.Родитель.ТекущиеДанные;
	ДанныеФормыТипСписокВыбора = Элемент.Родитель.ПодчиненныеЭлементы.ДанныеФормыТип.СписокВыбора;
	
	Если ТекущиеДанные.Тип = ДанныеФормыТипСписокВыбора[0].Значение Тогда // Текст
		Возврат;
	КонецЕсли;
	...
	ФайлПоля = Новый Файл(ТекущиеДанные.Значение);
	Если НЕ ВыполнятьНаСервере И НЕ Ждать ФайлПоля.СуществуетАсинх() Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеДанные.ТипКонтента = КлиентHTTPСлужебный.ТипMIMEРасширенияФайла(ФайлПоля.Расширение);
КонецПроцедуры

&НаКлиенте
Асинх Процедура ДанныеФормыИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекущиеДанные = Элемент.Родитель.ТекущиеДанные;
	Если ПустаяСтрока(ТекущиеДанные.Значение) Тогда
		Возврат;
	...
	ТекущиеДанные.ИмяФайла = ФайлПоля.Имя;
КонецПроцедуры
#КонецОбласти
...
Асинх Функция ДанныеФормыТелаЗапросаНаКлиенте()
	...
	Для Каждого Стр Из ТелоЗапросаДанныеФормы Цикл
		...
		ТипMIME = СокрЛП(Стр.ТипКонтента);
		
		Если Стр.Тип = ТипыДанныхФормы[0].Значение Тогда // Текст
			КлиентHTTPКлиентСервер.ДобавитьПолеФормыТекст(фРезультат, Стр.Ключ, Стр.Значение, ?(ПустаяСтрока(ТипMIME), Неопределено, ТипMIME));
		Иначе // Файл
			...
			ИмяФайла = СокрЛП(Стр.ИмяФайла);
			
			КлиентHTTPКлиентСервер.ДобавитьПолеФормыФайл(
				фРезультат,
				Стр.Ключ,
				ФайлПоля,
				?(ПустаяСтрока(ИмяФайла), Неопределено, ИмяФайла),
				?(ПустаяСтрока(ТипMIME), Неопределено, ТипMIME)
			);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПолеФормыCurlКоманды(ЧастиКоманды, Знач Поле)
	...
	Если Поле.Тип = ТипыДанныхФормыФайл Тогда
		Если ПустаяСтрока(Поле.Значение) Тогда
			Возврат;
		КонецЕсли;
		
		ЧастиПоляФормы.Добавить("@");
		
		Если НЕ ПустаяСтрока(Поле.ИмяФайла) Тогда
			ИмяФайла = Поле.ИмяФайла;
		КонецЕсли;
		ЭкранироватьОбратныйСлеш = ?(ВыполнятьНаСервере, РазделительПутиСервера(), ПолучитьРазделительПутиКлиента()) <> "\";
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(Поле.ТипКонтента) Тогда
		ТипMIME = Поле.ТипКонтента;
	КонецЕсли;
...
Процедура ОбработатьЧастьКомандыCurl(ЧастиКоманды, ЧастьКоманды, Опция, СобиратьТокен, ЧастиТокена, ОткрывающийСимвол, Отказ, ОписаниеОшибки)
	...
	Если ОткрывающийСимвол = "" ИЛИ СтрЗаканчиваетсяНа(ЧастьКоманды, ОткрывающийСимвол) И НЕ СтрЗаканчиваетсяНа(ЧастьКоманды, "\" + ОткрывающийСимвол) И НЕ СтрЗаканчиваетсяНа(ЧастьКоманды, "'\''") Тогда
		Если Опция = Неопределено Тогда
...
Процедура ОбработатьПолеФормыCurl(ДанныеИмпорта, Знач Опция, Отказ, ОписаниеОшибки)
	...
	Если ИндексТипаПоля = 1 Тогда
		Значение = Сред(Значение, 2);
	КонецЕсли;
	ОткрывающийСимвол = Лев(Значение, 1);
	Если ОткрывающийСимвол = """" ИЛИ ОткрывающийСимвол = "'" Тогда
		Значение = СтрЗаменить(Сред(Значение, 2, СтрДлина(Значение)-2), "\" + ОткрывающийСимвол, ОткрывающийСимвол);
	КонецЕсли;
...
Процедура ОбработатьЗаголовокCurl(ДанныеИмпорта, Знач Опция)
	...
	Значение = СокрЛ(СтрСоединить(ЧастиПоля, ":"));
