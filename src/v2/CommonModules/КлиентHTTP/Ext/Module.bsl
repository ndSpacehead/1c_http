...
//
Функция ОтветHawkКорректен(Знач ЗначениеЗаголовка, Знач КонтекстHawk, Знач ДанныеАдресВХ, ОписаниеОшибки) Экспорт
	...
	Части = Новый Массив;
	Части.Добавить("hawk.1.response");
	Части.Добавить(КонтекстHawk.ВременнаяМетка);
	Части.Добавить(КонтекстHawk.РазовоеСлово);
	Части.Добавить(КонтекстHawk.Метод);
	Части.Добавить(КонтекстHawk.Путь);
	Части.Добавить(НРег(КонтекстHawk.Хост)); 
	Части.Добавить(XMLСтрока(КонтекстHawk.Порт));
	Части.Добавить(ПодписьТела);
	Части.Добавить(ДанныеСервера.Расширение); // ext
	Части.Добавить("");
	
	Подпись = Base64Строка(HMAC256(
		ПолучитьДвоичныеДанныеИзСтроки(КонтекстHawk.Ключ),
		ПолучитьДвоичныеДанныеИзСтроки(СтрСоединить(Части, Символы.ПС))
	));
...
Функция ДанныеЗаголовкаОтветаHawk(Знач ЗначениеЗаголовка)
	фРезультат = Новый Структура("Подпись, ПодписьТела, Расширение", "", "", "");
	...
	Если ПозицияНачала > 0 Тогда
		...
		ПозицияОкончания = СтрНайти(ЗначениеЗаголовка, """", , ПозицияНачала);
		Если ПозицияОкончания > 0 Тогда
			фРезультат.Расширение = Сред(ЗначениеЗаголовка, ПозицияНачала, ПозицияОкончания - ПозицияНачала);
		КонецЕсли;
	КонецЕсли;
