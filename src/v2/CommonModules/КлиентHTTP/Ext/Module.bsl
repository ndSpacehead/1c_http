...
//
Функция ЗаголовокHawk(Знач Метод, Знач Конфигурация, Знач ДанныеАдресВХ = Неопределено) Экспорт
	фРезультат = Новый Массив;
	...
	Если ДанныеАдресВХ <> Неопределено Тогда
		...
		ПодписьТела = ПодписьТелаHawk(
			...
			КлиентHTTPКлиентСервер.КодировкаИзДопПараметров(Конфигурация.ДополнительныеПараметры)
		);
	КонецЕсли;
	...
	ЧастиСтроки.Добавить(Метод);
	ЧастиСтроки.Добавить(Конфигурация.ДанныеURI.Путь);
	ЧастиСтроки.Добавить(Конфигурация.ДанныеURI.Сервер); 
...
Функция ПодписьТелаHawk(Знач Данные, Знач ТипКонтента, Знач Кодировка)
	Если Данные.Размер() = 0 ИЛИ НЕ ЗначениеЗаполнено(ТипКонтента) Тогда
		Возврат "";
	КонецЕсли;
	
	Поток = Новый ПотокВПамяти;
	Запись = Новый ЗаписьДанных(Поток, ?(ЗначениеЗаполнено(Кодировка), Кодировка, КодировкаТекста.UTF8));
	
	Запись.ЗаписатьСтроку("hawk.1.payload");
	Запись.ЗаписатьСтроку(ТипКонтента);
