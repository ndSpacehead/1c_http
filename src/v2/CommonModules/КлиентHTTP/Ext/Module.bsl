...
Функция ЗаголовкиAWS4(Знач Метод, Знач Конфигурация, Знач ДанныеАдресВХ) Экспорт
	...
	ЧастиСтроки.Добавить(Конфигурация.ДанныеURI.АдресРесурса);
	ЧастиСтроки.Добавить(КаноническиеПараметрыAWS4(Конфигурация.ДанныеURI.Параметры));
...
Функция КаноническиеПараметрыAWS4(Знач ПараметрыЗапроса)
	...
	Для Каждого Параметр Из ПараметрыЗапроса Цикл
		...
		Для Каждого Значение Из Параметр.Значение Цикл
			СписокПараметров.Добавить(Параметр.Ключ, Значение);
		КонецЦикла;
	КонецЦикла;
	...
	Для Каждого Параметр Из СписокПараметров Цикл
		фРезультат.Добавить(
			?(ПустаяСтрока(Параметр.Представление), Параметр.Значение, Параметр.Значение + "=" + Параметр.Представление)
		);
	КонецЦикла;
