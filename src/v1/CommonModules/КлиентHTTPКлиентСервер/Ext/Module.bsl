...
//  ОбщийМодуль.КлиентHTTPКлиентСервер - модуль вызова функции
//
Функция УстановитьHawkАвторизацию(ДополнительныеПараметры, Знач Идентификатор, Знач Ключ) Экспорт
	ДополнительныеПараметры.Вставить("Аутентификация", ОбъектАутентификацииHawk(Идентификатор, Ключ));
...
Функция ПараметрыЗапросаСтрокой(Знач Параметры) Экспорт
	...
	Для Каждого КЗ Из Параметры Цикл
		ИмяПараметра = КлиентHTTPСлужебный.ФорматированноеИмяПараметраЗапроса(КЗ.Ключ);
		Если ПустаяСтрока(ИмяПараметра) Тогда
			Продолжить;
		КонецЕсли;
		Если КЗ.Значение.Количество() = 0 Тогда
			фРезультат.Добавить(ИмяПараметра);
			Продолжить;
		КонецЕсли;
		
		Для Каждого ЗначениеПараметра Из КЗ.Значение Цикл
			ФЗП = КлиентHTTPСлужебный.ФорматированноеЗначениеПараметраЗапроса(ЗначениеПараметра);
...
Процедура ДобавитьКодировкуВЗаголовкиЗапроса(Заголовки, Знач ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Свойство("Кодировка") И ТекущееЗначение <> Неопределено Тогда
		Заголовки.Вставить(КлиентHTTPПовтИсп.ЗаголовокТипКонтента(), ТекущееЗначение + "; charset=" + КодировкаИзДопПараметров(ДополнительныеПараметры));
	КонецЕсли;
КонецПроцедуры
...
Функция КонтекстАутентификацииHawk(Контекст, Конфигурация)
	...
	Возврат Новый ФиксированнаяСтруктура(
		"Ключ, Метод, Хост, Порт, Путь, ВременнаяМетка, РазовоеСлово",
		Аутентификация.Ключ,
		Контекст.Метод,
...
Функция ОбъектАутентификацииHawk(Знач Идентификатор, Знач Ключ)
	Возврат Новый ФиксированнаяСтруктура("Тип, Идентификатор, Ключ", "Hawk", Идентификатор, Ключ);
КонецФункции
