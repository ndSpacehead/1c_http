		...
		Если ПоляТелаЗапросаИмпорт.Количество() = 1 Тогда
			...
			Если ПолеТелаЗапросаИмпорт.Значение = Неопределено Тогда
				Если ПолеТелаЗапросаИмпорт.Тип = 0 Тогда // Это текст
					ТелоЗапросаИмпорт.Тип = Элементы.ТипТелаЗапроса.СписокВыбора[3]; // raw
					ТелоЗапросаИмпорт.Значение = ПолеТелаЗапросаИмпорт.Ключ;
				Иначе // Это файл
					ТелоЗапросаИмпорт.Тип = Элементы.ТипТелаЗапроса.СписокВыбора[4]; // file
					ТелоЗапросаИмпорт.Значение = ПолеТелаЗапросаИмпорт.Ключ;
				КонецЕсли;
			Иначе
				ТелоЗапросаИмпорт.Тип = Элементы.ТипТелаЗапроса.СписокВыбора[2]; // x-www-form-urlencoded
				ТелоЗапросаИмпорт.Значение = ПоляТелаЗапросаИмпорт;
			КонецЕсли;
		Иначе
			ТелоЗапросаИмпорт.Тип = Элементы.ТипТелаЗапроса.СписокВыбора[2]; // x-www-form-urlencoded
			ТелоЗапросаИмпорт.Значение = ПоляТелаЗапросаИмпорт;
