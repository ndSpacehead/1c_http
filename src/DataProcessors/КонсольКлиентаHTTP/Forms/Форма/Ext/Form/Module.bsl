...
Процедура ОтобразитьРазмерТелаОтвета()
	...
	Если РазмерОтвета > 1073741823 Тогда
		Сдвиг = 9;
		Единицы = "ГБ";
	ИначеЕсли РазмерОтвета > 1048575 Тогда
		Сдвиг = 6;
		Единицы = "МБ";
	ИначеЕсли РазмерОтвета > 1023 Тогда
		Сдвиг = 3;
		Единицы = "КБ";
	Иначе
		Точность = 0;
	...
	Элементы.РазмерОтвета.Формат = СтрШаблон("ЧДЦ=%1; ЧС=%2; ЧН='0 Б'; ЧГ=; ЧФ='Ч %3'", Точность, Сдвиг, Единицы);
КонецПроцедуры
...
Процедура ВывестиТелоОтветаJSON(Знач Текст)
	...
	Иначе
		...
		Запись.УстановитьСтроку(Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет, "", Истина));
		ЗаписатьJSON(Запись, ОбъектJSON);
		Текст = Запись.Закрыть();
	КонецЕсли;
	
	ЧастиТекста = Новый Массив;
	ЧастиТекста.Добавить(ТекстHTMLОтображенияJSONНачало());
	ЧастиТекста.Добавить(Текст);
	ЧастиТекста.Добавить(ТекстHTMLОтображенияJSONОкончание());
	
	ОтветJSON = СтрСоединить(ЧастиТекста);
КонецПроцедуры
...
&НаКлиенте
Функция ТекстHTMLОтображенияJSONНачало()
	Возврат
		"<!DOCTYPE html>
		|<html>
		|<head>
...
&НаКлиенте
Функция ТекстHTMLОтображенияJSONОкончание()
	Возврат
		"');
		|			document.write(JSON.stringify(o, null, '\t'));
		|		}
		|		catch(e) {
		|			document.write('не удалось прочитать как JSON');
		|		}
		|</script>
		|	</pre>
		|</body>
