...
Процедура ВыполнитьЗапрос(Команда)
	...
	ОтобразитьКодСостоянияСервера(ОтветHTTP.КодСостояния);
	
	ДанныеТелаОтвета = ОтветHTTP.Тело;
	ТипMIMEОтвета = КлиентHTTPКлиентСервер.ТипMIMEИзЗаголовков(ОтветHTTP.Заголовки);
...
Процедура ИспользоватьПроксиПриИзменении(Элемент)
	...
	Элементы.ПарольПрокси.Доступность       = ИспользоватьПрокси;
...
#Область Заполнение_запроса
	...
	ТипАвторизации = ОбъектИстории.Авторизация.Тип;
	ПользовательАвторизации = ОбъектИстории.Авторизация.Пользователь;
	ПарольАвторизации = ОбъектИстории.Авторизация.Пароль;
	ТокенАвторизации = ОбъектИстории.Авторизация.Токен;
...
#Область Заполнение_ответа
	ОтобразитьКодСостоянияСервера(ОбъектИстории.КодСостояния);
	
	ВремяВыполнения = ОбъектИстории.ВремяВыполнения;
	
	ФроматированиеВремениВыполненияЗапроса();
	
	ДанныеТелаОтвета = ОбъектИстории.Тело;
	ТипMIMEОтвета = КлиентHTTPКлиентСервер.ТипMIMEИзЗаголовков(ОбъектИстории.ЗаголовкиОтвета);
	
	ОтобразитьРазмерТелаОтвета();
	
	ЗаголовкиОтвета.Очистить();
	Для Каждого КЗ Из ОбъектИстории.ЗаголовкиОтвета Цикл
		ЗаполнитьЗначенияСвойств(ЗаголовкиОтвета.Добавить(), КЗ);
	КонецЦикла;
	ЗаголовкиОтвета.Сортировать("Ключ");
	
	ИмяФайлаТела = ОбъектИстории.ИмяФайлаТела;
	КодировкаТелаОтвета = КлиентHTTPКлиентСервер.КодировкаИзЗаголовков(ОбъектИстории.ЗаголовкиОтвета);
...
Процедура ДобавитьВИсторию(Знач Ответ)
	...
	ОбъектИстории.Вставить("КодСостояния", Ответ.КодСостояния);
	ОбъектИстории.Вставить("Тело",Ответ.Тело);
	ОбъектИстории.Вставить("ЗаголовкиОтвета", Ответ.Заголовки);
	ОбъектИстории.Вставить("ИмяФайлаТела", Ответ.ИмяФайлаТела);
	ОбъектИстории.Вставить("Авторизация", Новый Структура("Тип, Пользователь, Пароль, Токен", ТипАвторизации, ПользовательАвторизации, ПарольАвторизации, ТокенАвторизации));
