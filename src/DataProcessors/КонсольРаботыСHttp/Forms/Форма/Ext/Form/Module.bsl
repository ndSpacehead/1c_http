...
Процедура ВыполнитьЗапрос(Команда)
	...
	ИначеЕсли Схема = Элементы.Схема.СписокВыбора[1].Значение Тогда // POST
		...
		ИначеЕсли ТипТелаЗапроса = ТипТелаЗапросаСписокВыбора[3].Значение Тогда // binary
			ОтветHTTP = РаботаСHttp.ОтправитьФайл(ИдентификаторРесурса, Новый Файл(ФайлТелаЗапроса), ДП, ПЗ);
		КонецЕсли; 
...
Функция ДанныеФормыТелаЗапроса()
	...
	ТипыДанныхФормы = Элементы.ДанныеФормы.ПодчиненныеЭлементы.ДанныеФормыТип.СписокВыбора;
	
	Для Каждого Стр Из ДанныеФормы Цикл
		Если НЕ Стр.Активно Тогда
			Продолжить;
		КонецЕсли;
		...
		Иначе // Файл
			...
			Поле.Значение = ФайлПоля;
			Поле.Вставить("Файл", Новый Структура("Имя, ТипMIME", ФайлПоля.Имя, ТипMIME));
		КонецЕсли;
		
		фРезультат.Добавить(Поле);
	КонецЦикла;
...
Функция ПараметрыОтправкиДанныхФормы(Знач ДополнительныеПараметры)
	...
	фРезультат.Вставить("Разделитель", Разделитель);
...
Процедура ВыборФайлаТелаЗапросаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	Если ВыбранныеФайлы <> Неопределено Тогда
		// TODO: переименовать ФайлТелаЗапроса в ТелоЗапросаФайл 
...
// TODO: переименовать ТЗ ДанныеФормы в ТелоЗапросаДанныеФормы
