...
Функция Получить(Знач ИдентификаторРесурса, Знач ПараметрыЗапроса = Неопределено, Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	ОбработатьОтвет(Ответ, ДополнительныеПараметры);
...
Процедура ОбработатьОтвет(Знач Ответ, ДополнительныеПараметры)
	Если НЕ ДополнительныеПараметры.Свойство("Сессия") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Заголовок Из Ответ.Заголовки Цикл
		Если НРег(Заголовок.Ключ) = "set-cookie" Тогда
			ПринятьПеченье(ДополнительныеПараметры.Сессия, Заголовок.Значение);
			Прервать;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура ПринятьПеченье(Сессия, Знач ОписаниеПеченья)
	Перем Печенье;
	...
	Для Каждого Печенько Из ВыпеченныеПеченьки(ОписаниеПеченья) Цикл
		...
		Получатель = ?(Ячейка = Неопределено, "", Ячейка.Значение);
		ПеченьеСорта = Печенье.Получить(Получатель);
		Если ПеченьеСорта = Неопределено Тогда
			ПеченьеСорта = Новый Соответствие;
			Печенье.Вставить(Получатель, ПеченьеСорта);
		КонецЕсли;
		
		ПеченьеСорта.Вставить(НРег(Печенько.Имя), Печенько);
	КонецЦикла;
КонецПроцедуры

Функция ВыпеченныеПеченьки(Знач ОписаниеПеченья)
	фрезультат = Новый Массив;
	Рецепты = Новый Массив;
	Рецепт = "";

	Для Каждого Кусок Из СтрРазделить(ОписаниеПеченья, ",", Ложь) Цикл
		ПозицияРавно = СтрНайти(Кусок, "=");
		ПозицияРазделителя = СтрНайти(Кусок, ";");
		Если ПозицияРавно = 0 ИЛИ ПозицияРавно > ПозицияРазделителя Тогда
			Рецепт = Рецепт + "," + Кусок;
		Иначе
			Рецепты.Добавить(Рецепт);
			Рецепт = Кусок;
		КонецЕсли;
	КонецЦикла;
	Рецепты.Удалить(0);
	...
	Для Каждого Рецепт Из Рецепты Цикл
		Ингредиенты = СтрРазделить(Рецепт, ";", Ложь);
		Ингредиент = СтрРазделить(Ингредиенты[0], "=", Ложь);
		Печенько = Новый Структура("Имя, Значение, Атрибуты", СокрЛП(Ингредиент[0]), ?(Ингредиент.Количество() > 1, СокрЛП(Ингредиент[1]), ""), Новый Соответствие);
		
		Для я = 1 По Ингредиенты.ВГраница() Цикл
			Ингредиент = СтрРазделить(Ингредиенты[я], "=");
			Печенько.Атрибуты.Вставить(СокрЛП(НРег(Ингредиент[0])), Новый Структура("Имя, Значение", СокрЛП(Ингредиент[0]), ?(Ингредиент.Количество() > 1, СокрЛП(Ингредиент[1]), "")));
		КонецЦикла;
		
		фрезультат.Добавить(Печенько);
	КонецЦикла;
