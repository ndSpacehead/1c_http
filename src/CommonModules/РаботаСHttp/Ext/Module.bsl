...
//     HTTPОтвет
//
// Параметры тела запроса. Ключи:
//     Наименование - Строка - имя поля
//     Значение - Строка - значение поля
//     ТипMIME - Строка - тип значения поля (необязательный)
//     Файл - Структура - (все ключи являются необязательными) ключи: ИмяФайла, ТипMIME, КодировкаФайла, ИспользоватьПуть - наличие ключа
//            "ИспользоватьПуть" указывает на добавление в тело запроса файла в бинарном представлении по указанному пути в значении по ключу "Значение",
//            иначе значение ключа "Значение" содержит текстовое представление файла (например, Base64)
//
Функция ОтправитьДанныеФормы(Знач ИдентификаторРесурса, Знач Данные, Знач ДополнительныеПараметры = Неопределено, Знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	Для Каждого ЭлементФормы Из Данные Цикл
		...
		Если ЭлементФормы.Свойство("Файл") Тогда
			...
			КонецЕсли;
			Если ЭлементФормы.Файл.Свойство("КодировкаФайла") Тогда
				Запись.ЗаписатьСтроку("Content-Transfer-Encoding: " + ЭлементФормы.Файл.КодировкаФайла);
			КонецЕсли;
...
Процедура ДобавитьРазмерДанныхВЗаголовкиЗапроса(Заголовки, Знач ДополнительныеПараметры, Знач Данные)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДвоичныеДанные") Тогда
		Заголовки.Вставить("Content-Lenght", Данные.Размер());
	Иначе
