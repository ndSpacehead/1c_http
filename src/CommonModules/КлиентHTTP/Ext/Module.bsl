...
Процедура ОбработатьПереходноеСостояниеДКА(Печеньки, СостояниеДКА, Действие, СостояниеЧтения, ОписанияПеченья, СостояниеПеченьки, СостояниеАтрибута, СледующаяПозиция)
	...
	Если Действие.СобиратьЗначениеПеченьки Тогда
		...
		ИначеЕсли СостояниеЧтения.Токен.ТипТокена = ТипыТокенов.РазделителиПеченья Тогда // добавить фрагмент значения печеньки
			СостояниеПеченьки.Построитель.Добавить(ЧастьОписанияПеченья(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция));
	...
	ИначеЕсли Действие.СобиратьЗначениеАтрибута Тогда
		...
		ИначеЕсли СостояниеЧтения.Токен.ТипТокена = ТипыТокенов.РазделителиПеченья Тогда  // добавить фрагмент значения атрибута
			...
			СостояниеАтрибута.Построитель.Добавить(ЧастьОписанияПеченья(ОписанияПеченья.Нормализованное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция));
			...
			Если СостояниеЧтения.КоличествоЗапятых <> Неопределено И СостояниеЧтения.ПрочитаноЗапятых >= СостояниеЧтения.КоличествоЗапятых Тогда
	...
	Если Действие.ЗаписатьИмяПеченьки Тогда
		СостояниеПеченьки.Объект.Имя = СокрЛП(ЧастьОписанияПеченья(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция));
...
Процедура ОбработатьОчередноеСостояниеДКА(Печеньки, СостояниеДКА, Действие, СостояниеЧтения, ОписанияПеченья, СостояниеПеченьки, СостояниеАтрибута)
	...
	ИначеЕсли СостояниеДКА.Очередное = СостоянияДКА.ЗначениеПеченьки Тогда
		Если Действие.ЗаписатьЗначениеПеченьки Тогда
			СостояниеПеченьки.Объект.Значение = СостояниеПеченьки.Объект.Значение + ЧастьОписанияПеченья(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция);
	...
	ИначеЕсли СостояниеДКА.Очередное = СостоянияДКА.ИмяИлиЗначениеПеченьки Тогда
		...
		СостояниеПеченьки.Построитель.Добавить(СостояниеПеченьки.Объект.Значение + ЧастьОписанияПеченья(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция));
		СостояниеПеченьки.Построитель.Добавить(СостояниеЧтения.Токен.Токен);
		СостояниеЧтения.Позиция = СостояниеЧтения.Токен.Позиция + 1;
		ПереключитьСостояниеДКА(СостояниеДКА);
	ИначеЕсли СостояниеДКА.Очередное = СостоянияДКА.ИмяПеченькиИлиЗначениеАтрибута Тогда
		...
		ПереключитьСостояниеДКА(СостояниеДКА);
	Иначе	
		ВызватьИсключение "Не удалось разобрать зоголовок Set-Cookie";
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработатьФинальноеСостоянияДКА(Печеньки, ФинальноеДействие, СостояниеЧтения, Знач ОписанияПеченья, СостояниеПеченьки, СостояниеАтрибута)
	Если ФинальноеДействие.ЗаписатьЗначениеПеченьки Тогда
		СостояниеПеченьки.Объект.Значение = Сред(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция - СостояниеЧтения.Позиция);
		Печеньки.Добавить(СостояниеПеченьки.Объект);
	ИначеЕсли ФинальноеДействие.СобиратьЗначениеПеченьки Тогда
		СостояниеПеченьки.Построитель.Добавить(Сред(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция - СостояниеЧтения.Позиция));
		СостояниеПеченьки.Объект.Значение = СтрСоединить(СостояниеПеченьки.Построитель);
		Печеньки.Добавить(СостояниеПеченьки.Объект);
	ИначеЕсли ФинальноеДействие.ЗаписатьАтрибут Тогда
		СостояниеАтрибута.Объект.Значение = Сред(ОписанияПеченья.Нормализованное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция - СостояниеЧтения.Позиция);
		СостояниеПеченьки.Объект.Атрибуты.Добавить(СостояниеАтрибута.Объект);
		Печеньки.Добавить(СостояниеПеченьки.Объект);
	ИначеЕсли ФинальноеДействие.СобиратьЗначениеАтрибута Тогда
		ФинальноеДействие.ЗаписатьИмяПеченьки = (СостояниеЧтения.КоличествоЗапятых <> Неопределено И СостояниеЧтения.ПрочитаноЗапятых >= СостояниеЧтения.КоличествоЗапятых);
		
		Если ФинальноеДействие.ЗаписатьИмяПеченьки Тогда
			СостояниеАтрибута.Построитель.Удалить(СостояниеАтрибута.Построитель.ВГраница());
		Иначе
			СостояниеАтрибута.Построитель.Добавить(Сред(ОписанияПеченья.Нормализованное, СостояниеЧтения.Позиция, СостояниеЧтения.Токен.Позиция - СостояниеЧтения.Позиция));
		КонецЕсли;
	...
	Если ФинальноеДействие.ЗаписатьИмяПеченьки Тогда
		СостояниеПеченьки.Объект.Имя = СокрЛП(Сред(ОписанияПеченья.Исходное, СостояниеЧтения.Позиция, ОписанияПеченья.Длина - СостояниеЧтения.Позиция + 1));
		Печеньки.Добавить(СостояниеПеченьки.Объект);
	КонецЕсли;
КонецПроцедуры

Функция ЧастьОписанияПеченья(ОписаниеПеченьки, ПозицияЧтения, ПозицияТокена)
	Возврат Сред(ОписаниеПеченьки, ПозицияЧтения, ПозицияТокена - ПозицияЧтения);
КонецФункции
	...
	Для Каждого Атрибут Из Атрибуты Цикл
		...
		Если ИмяСвойства = ИмяСрок Тогда
			...
			Если ВременнаяМетка > 0 Тогда
				фРезультат.Вставить(ИмяСвойства, '19700101' + ВременнаяМетка);
				Продолжить;
			КонецЕсли;
