...
Функция ОтправитьТекст(Знач ИдентификаторРесурса, Знач Данные, Знач ДополнительныеПараметры = Неопределено, Знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	Возврат ВыполнитьЗапросHTTP(РаботаСHttpПовтИсп.МетодPOST(), Конфигурация, Данные);
КонецФункции
...
Функция ЗаписатьТекст(Знач ИдентификаторРесурса, Знач Данные, Знач ДополнительныеПараметры = Неопределено, Знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	Возврат ВыполнитьЗапросHTTP(РаботаСHttpПовтИсп.МетодPUT(), Конфигурация, Данные);
КонецФункции
...
Функция Удалить(Знач ИдентификаторРесурса, Знач Данные = "", Знач ДополнительныеПараметры = Неопределено, Знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	Возврат ВыполнитьЗапросHTTP(РаботаСHttpПовтИсп.МетодDELETE(), Конфигурация, Данные);
КонецФункции
...
Функция ВыполнитьЗапросHTTP(Знач Метод, Знач Конфигурация, Знач Данные = Неопределено, Знач НомерПеренаправления = 0)
	...
	Если Метод = РаботаСHttpПовтИсп.МетодPOST() ИЛИ Метод = РаботаСHttpПовтИсп.МетодPUT() ИЛИ Метод = РаботаСHttpПовтИсп.МетодDELETE() Тогда
...
Функция ОбработанныйОтвет(Знач Ответ, Знач Конфигурация, Знач КонтекстВыполнения)
	...
	Если Ответ.КодСостояния = РаботаСHttpПовтИсп.КодСостоянияНеАвторизовано() И Конфигурация.ДополнительныеПараметры.Свойство("АвторизоватьсяDigest") Тогда
...
Функция СвойстваПеченьки(Знач Имя, Знач Значение, Знач Атрибуты)
	...
	АтрибутКакСвойство = РаботаСHttpПовтИсп.АтрибутыКакСвойства();
	ИмяСрок = РаботаСHttpПовтИсп.СвойствоПеченькаСрок();
	ИмяВремяАктуальности = РаботаСHttpПовтИсп.СвойствоПеченькаВремяАктуальности();
	ИмяАдресРесурса = РаботаСHttpПовтИсп.СвойствоПеченькаАдресРесурса();
	ИмяЗащищенноеСоединение = РаботаСHttpПовтИсп.СвойствоПеченькаЗащищенноеСоединение();
	...
	Для Каждого Атрибут Из Атрибуты Цикл
		...
		Если ИмяСвойства = ИмяСрок Тогда
			Срок = РаботаСHttpСлужебный.ДатаИзHTTP(Атрибут.Значение, Ошибка);
		...
		ИначеЕсли ИмяСвойства = ИмяВремяАктуальности Тогда
			...
			Если НЕ Ошибка Тогда
				фРезультат.Вставить(ИмяСрок, РаботаСHttpВызовСервера.ТекущаяУниверсальнаяДатаНаСервере() + ВремяАктуальности);
			КонецЕсли;
...
Функция ПеченьеДляURI(Знач ДанныеURI, Знач Печенье)
	...
	ДатаЗапроса = РаботаСHttpВызовСервера.ТекущаяУниверсальнаяДатаНаСервере();
...
Функция ИдентификаторПеренаправления(Знач КодСостояния, Знач Заголовки)
	Возврат ?(
		РаботаСHttpПовтИсп.КодыСостоянияПеренаправления().Получить(КодСостояния) = Неопределено,
		Неопределено,
		РаботаСHttpВызовСервера.РаскодированныйИдентификаторРесурса(ЗначениеЗаголовка("location", Заголовки))
	);
КонецФункции
...
Функция ЗаголовокDigest(Знач Конфигурация, Знач КонтекстВыполнения)
	...
	HA1 = РаботаСHttpСлужебный.ХешСтрока(A1, АлгоритмХеширования);
	Если АлгоритмХеширования = "md5-sess" Тогда
		HA1 = РаботаСHttpСлужебный.ХешСтрока(СтрШаблон("%1:%2:%3", HA1, Параметры.nonce, УникальныйКлючКлиента), АлгоритмХеширования);
	КонецЕсли;
	
	A2 = ?(
	    КачествоЗащиты = "auth-int",
		СтрШаблон("%1:%2:%3", КонтекстВыполнения.Метод, АдресРесурсаСПараметрами, РаботаСHttpСлужебный.ХешСтрока(КонтекстВыполнения.Данные, АлгоритмХеширования)),
		СтрШаблон("%1:%2", КонтекстВыполнения.Метод, АдресРесурсаСПараметрами)
	);
	HA2 = РаботаСHttpСлужебный.ХешСтрока(A2, АлгоритмХеширования);
	...
	Ответ = РаботаСHttpСлужебный.ХешСтрока(
		?(
			КачествоЗащиты = Неопределено,
