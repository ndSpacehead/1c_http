...
//  * Тело - ДвоичныеДанные, Неопределено - данные тела HTTP-ответа,
...
// Установка дополнительного параметра ТелоОтветаКакТекст
//	При получении ответа - будет пытаться парсить тело в текст
// Параметры:
//  ДополнительныеПараметры	 - Структура - конфигурация выполнения запроса
// 
// Возвращаемое значение:
...
Функция ТелоОтветаКакТекст(ДополнительныеПараметры) Экспорт
	УстановитьДополнительныйПараметр(ДополнительныеПараметры, "ТелоОтветаКакТекст", Истина);
...
Процедура РазобратьОснованиеИдентификатораРесурса(ДанныеURI, Знач ИдентификаторРесурса, Знач ПозицияНачала, ПозицияОкончания)
	...
	ПозицияОкончания = СтрНайти(ИдентификаторРесурса, "?", , ПозицияНачала); // допущение: пароль не содержит символ '?'
	...
	Если ДлинаАвторизации > 0 Тогда
		...
		ДанныеURI.Пользователь = СокрЛП(Лев(Авторизация, ПозицияРазделителяАвторизации - 1));
		ДанныеURI.Пароль       = Прав(Авторизация, ДлинаАвторизации - ПозицияРазделителяАвторизации); 	
	КонецЕсли;
...
Функция ОбъектОбработанногоОтвета(Знач Ответ, Знач ДополнительныеПараметры)
	...
	ТелоОтвета = ТелоОтвета(Ответ, ДополнительныеПараметры);
...
Функция ТелоОтвета(Знач Ответ, Знач ДополнительныеПараметры)
	Перем ТелоОтвета, ТипСодержимого, ФорматСжатия;
	...
	Если ДополнительныеПараметры.Свойство("ТелоОтветаКакТекст") Тогда
		Если КлиентHTTPПовтИсп.ТипыMIMEТекстовыхДанных().Получить(НРег(ТипСодержимого)) <> Неопределено Тогда
			ТелоОтвета = Ответ.ПолучитьТелоКакСтроку();
			Возврат ТелоОтвета;
		КонецЕсли;
	КонецЕсли;
	
	Если ФорматСжатия = Неопределено Тогда
		ТелоОтвета = Ответ.ПолучитьТелоКакДвоичныеДанные();
	Иначе
		ТелоОтвета = Декомпрессия(Ответ.ПолучитьТелоКакПоток());
	КонецЕсли;
