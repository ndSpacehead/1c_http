...
Функция ОтправитьТекст(Знач ИдентификаторРесурса, Знач Данные, Знач ДополнительныеПараметры = Неопределено, Знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	Если НЕ ДП.Свойство("ТипMIME") Тогда
		ДП.Вставить("ТипMIME", КлиентHTTPПовтИсп.ТипMIMEText());
	КонецЕсли;
	Если НЕ ДП.Свойство("Кодировка") Тогда
		ДП.Вставить("Кодировка", КлиентHTTPПовтИсп.КодировкаПоУмолчанию());
	КонецЕсли;
	
	Возврат ОтправитьДвоичныеДанные(
		ИдентификаторРесурса,
		ПолучитьДвоичныеДанныеИзСтроки(Данные, ДП.Кодировка),
		ДП,
		ПараметрыЗапроса
	);
КонецФункции
...
Функция ОтправитьФайл(Знач ИдентификаторРесурса, Знач Данные, Знач ДополнительныеПараметры = Неопределено, Знач ПараметрыЗапроса = Неопределено) Экспорт
	...
	Если НЕ ДП.Свойство("ТипMIME") Тогда
		ДП.Вставить("ТипMIME", КлиентHTTPСлужебный.ТипMIMEРасширенияФайла(Данные.Расширение));
	КонецЕсли;
...
Функция КонфигурацияЗапроса(Знач ИдентификаторРесурса, Знач ПараметрыЗапроса = Неопределено, Знач ДополнительныеПараметры = Неопределено)
...
Функция ЗаголовкиЗапроса(Знач ДанныеURI, Знач ДополнительныеПараметры)
...
Функция ОбработанныйОтвет(Знач Ответ, Знач Конфигурация, Знач КонтекстВыполнения)
	...
	Если Печенье <> Неопределено Тогда
		ПринятьПеченье(Сессия, Печенье, Конфигурация.ДанныеURI);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НовыйИдентификаторРесурса) И КонтекстВыполнения.НомерПеренаправления <= Сессия.ПорогПеренаправлений Тогда
		...
		Конфигурация.Параметры = ПараметрыЗапросаСтрокой(Конфигурация.ДанныеURI, НовыеПараметрыЗапроса(), Конфигурация.ДополнительныеПараметры);
	...
	Если Ответ.КодСостояния = КлиентHTTPПовтИсп.КодСостоянияНеАвторизовано() И Конфигурация.ДополнительныеПараметры.Свойство("АвторизоватьсяDigest") Тогда
		...
		Если СтрНачинаетсяС(НРег(РецептАвторизации), "digest") Тогда
			...
			Конфигурация.Заголовки.Вставить("Authorization", ЗаголовокDigest(Конфигурация, КонтекстВыполнения));
...
Процедура ПринятьПеченье(Сессия, Знач ОписаниеПеченья, Знач ДанныеURI)
	...
	Для Каждого Печенько Из ВыпеченныеПеченьки(ОписаниеПеченья) Цикл
		...
		Получатель = НормализованныйАдресСервера(
...
Функция ВыпеченныеПеченьки(Знач ОписаниеПеченья)
	...
	Для Каждого ПозицияТокена Из КлиентHTTPВызовСервера.ПозицииТокеновПеченья(НормализованноеОписаниеПеченья) Цикл
...
Функция СвойстваПеченьки(Знач Имя, Знач Значение, Знач Атрибуты)
	...
	Для Каждого Атрибут Из Атрибуты Цикл
		...
		ИначеЕсли ИмяСвойства = ИмяВремяАктуальности Тогда
			...
			Если НЕ Ошибка Тогда
				фРезультат.Вставить(ИмяСрок, КлиентHTTPВызовСервера.ТекущаяУниверсальнаяДатаНаСервере() + ВремяАктуальности);
			КонецЕсли;
...
Функция НормализованныйАдресСервера(Знач АдресСервера)
	Возврат КодироватьАдресСервера(
		КодироватьАдресСервера(НРег(СокрЛП(АдресСервера)))
	);
КонецФункции
...
Функция КодироватьАдресСервера(Знач АдресСервера)
	...
	Для Каждого Домен Из СтрРазделить(АдресСервера, ".", Ложь) Цикл
		...
		Для я = 1 По СтрДлина(Домен) Цикл
			ЕстьНеASCII = НЕ КлиентHTTPСлужебный.ЭтоСимволASCII(КодСимвола(Домен, я));
		...
		фРезультат.Добавить(
			?(
				ЕстьНеASCII,
				Префикс + КлиентHTTPСлужебный.PunycodeЗначение(Домен),
				Домен
			)
		);
	КонецЦикла;
...
Функция ЗаголовокDigest(Знач Конфигурация, Знач КонтекстВыполнения)
	...
	HA1 = КлиентHTTPСлужебный.ХешСтрока(A1, АлгоритмХеширования);
	Если СтрЗаканчиваетсяНа(АлгоритмХеширования, "-sess") Тогда
		HA1 = КлиентHTTPСлужебный.ХешСтрока(СтрШаблон("%1:%2:%3", HA1, Параметры.nonce, УникальныйКлючКлиента), АлгоритмХеширования);
	КонецЕсли;
	
	A2 = ?(
	    КачествоЗащиты = "auth-int",
		СтрШаблон("%1:%2:%3", КонтекстВыполнения.Метод, АдресРесурсаСПараметрами, КлиентHTTPСлужебный.ХешСтрока(КонтекстВыполнения.Данные, АлгоритмХеширования)),
		СтрШаблон("%1:%2", КонтекстВыполнения.Метод, АдресРесурсаСПараметрами)
	);
	HA2 = КлиентHTTPСлужебный.ХешСтрока(A2, АлгоритмХеширования);
	...
	Ответ = КлиентHTTPСлужебный.ХешСтрока(
		?(
			КачествоЗащиты = Неопределено,
...
Функция Декомпрессия(Знач ПотокСжатыхДанных)
	...
	ОластиZIP = КлиентHTTPПовтИсп.ОластиФайлаВАрхивеZIP();
	Поток = Новый ПотокВПамяти(ОластиZIP.РазмерLFH + РазмерСжатыхДанных + ОластиZIP.РазмерDD + ОластиZIP.РазмерCDFH + ОластиZIP.РазмерEOCD);
	
	ОластиZIP.ОбластьLFH.Записать(30, ИмяФайлаВАрхиве);
	
	Чтение = Новый ЧтениеДанных(ПотокСжатыхДанных);
	Чтение.Пропустить(10); // размер префикса GZip
	
	Запись = Новый ЗаписьДанных(Поток);
	Запись.ЗаписатьБуферДвоичныхДанных(ОластиZIP.ОбластьLFH);
	...
	ОластиZIP.ОбластьDD.ЗаписатьЦелое32(4,  КонтрольнаяСумма);
	ОластиZIP.ОбластьDD.ЗаписатьЦелое32(8,  РазмерСжатыхДанных);
	ОластиZIP.ОбластьDD.ЗаписатьЦелое32(12, РазмерНесжатыхДанных);

	ОластиZIP.ОбластьCDFH.ЗаписатьЦелое32(16, КонтрольнаяСумма);
	ОластиZIP.ОбластьCDFH.ЗаписатьЦелое32(20, РазмерСжатыхДанных);
	ОластиZIP.ОбластьCDFH.ЗаписатьЦелое32(24, РазмерНесжатыхДанных);
	ОластиZIP.ОбластьCDFH.Записать(       46, ИмяФайлаВАрхиве);

	ОластиZIP.ОбластьEOCD.ЗаписатьЦелое32(16, ОластиZIP.РазмерLFH + РазмерСжатыхДанных + ОластиZIP.РазмерDD);
	
	Запись = Новый ЗаписьДанных(Поток);
	Запись.ЗаписатьБуферДвоичныхДанных(ОластиZIP.ОбластьDD);
	Запись.ЗаписатьБуферДвоичныхДанных(ОластиZIP.ОбластьCDFH);
	Запись.ЗаписатьБуферДвоичныхДанных(ОластиZIP.ОбластьEOCD);
	Запись.Закрыть();
