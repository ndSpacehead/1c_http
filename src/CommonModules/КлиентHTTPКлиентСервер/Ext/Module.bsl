...
//
Функция ОбъектИдентификатораРесурса(Знач Сервер, Знач Порт = Неопределено, Знач АдресРесурса = "", Знач ЗащищенноеСоединение = Истина) Экспорт
	...
	Возврат Новый Структура("Сервер, Порт, АдресРесурса, ЗащищенноеСоединение", Сервер, Порт, АдресРесурса, ЗащищенноеСоединение);
КонецФункции
#КонецОбласти
...
Функция ТелоОтвета(Знач Ответ, Знач ДополнительныеПараметры)
	...
	ИначеЕсли СпособЧтения.Формат = ФорматТелаОтветаJSON() Тогда
		...
		Чтение.УстановитьСтроку(ПолучитьСтрокуИзДвоичныхДанных(ДанныеТелаОтвета, Кодировка));
