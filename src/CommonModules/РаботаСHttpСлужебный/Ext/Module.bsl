...
Функция ЭтоСимволASCII(Знач КодСимвола) Экспорт
...
Функция ДатаИзHTTP(Знач ДатаHTTP, Ошибка) Экспорт
	...
	Попытка
		фРезультат = Дата(
			Год,
			Месяц + 1,
			Число(ЧастиДатыHTTP[1]),
			Число(ЧастиДатыHTTP[4]),
			Число(ЧастиДатыHTTP[5]),
			Число(ЧастиДатыHTTP[6])
		);
	Исключение
		Ошибка = Истина;
		фРезультат = Неопределено;
...
Функция ХешСтрока(Знач Данные, Знач Алгоритм) Экспорт
	Возврат РаботаСHttpВызовСервера.ХешСтрока(Данные, Алгоритм);
КонецФункции
